...
const GlassMaterial = {
  uniforms: { uTime: { value: 0 }, uColor: { value: new THREE.Color('#E0FFFF') } },
  ...
  fragmentShader: `uniform float uTime; uniform vec3 uColor; varying vec3 vNormal; varying vec3 vViewPosition; void main() { vec3 viewDir = normalize(vViewPosition); vec3 normal = normalize(vNormal); float fresnel = dot(viewDir, normal); fresnel = clamp(1.0 - fresnel, 0.0, 1.0); float rim = pow(fresnel, 3.0); float inner = pow(fresnel, 1.0) * 0.1; float flow = sin(normal.y * 10.0 + uTime * 2.0 + normal.x * 5.0); flow = smoothstep(0.0, 1.0, flow) * 0.15; float alpha = rim * 0.8 + inner + flow + 0.02; vec3 finalColor = mix(vec3(0.0, 0.1, 0.3), uColor * 0.5, rim); gl_FragColor = vec4(finalColor, alpha); }`
};
...
const CoreGlowMaterial = {
  ...
  fragmentShader: `uniform vec3 uColor; varying vec2 vUv; void main() { vec2 uv = vUv * 2.0 - 1.0; float dist = length(uv); float glow = 1.0 - smoothstep(0.0, 0.4, dist); glow = pow(glow, 2.0); if (glow < 0.01) discard; gl_FragColor = vec4(uColor * 2.0, glow); }`
};
const ShockwaveMaterial = {
  uniforms: { uTime: { value: 0 }, uColor: { value: new THREE.Color('#00FFFF') } },
  vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
  fragmentShader: `uniform float uTime; uniform vec3 uColor; varying vec2 vUv; void main() { vec2 uv = vUv * 2.0 - 1.0; float dist = length(uv); if (dist > 1.0) discard; float wave = fract(uTime * 1.5 - dist * 1.0); float ring = smoothstep(0.0, 0.1, wave) * smoothstep(0.4, 0.0, wave); ring *= (1.0 - dist); gl_FragColor = vec4(uColor, ring * 0.4); }`
};
